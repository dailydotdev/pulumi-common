...
function createSubscriptionsFromWorkers(name, workers, serviceUrl) {
    ...
    return workers.map((worker) => new gcp.pubsub.Subscription(`${name}-sub-${worker.subscription}`, Object.assign({ topic: worker.topic, name: worker.subscription, pushConfig: {
